...
export async function chatCommand(userInput: string) {
  ...
  if (!intent) intent = await interpreter.parse(userInput, walletContext);
  ...
  // ── Unlock wallet for sensitive actions ────────────────────────────────────
  ...
  if (intent.action === 'RESEARCH') {
    // researchCommand takes args: string[]
    const topic = intent.topic || intent.token;
    await researchCommand(topic ? [topic] : []);
    return;
  }
  ...
  if (intent.action === 'PORTFOLIO') {
    ...
    const portfolioBox = boxen(
      ...
      pTable.toString() +
      `\n\n${chalk.green.bold('TOTAL NET WORTH: $')}${chalk.green.bold(
